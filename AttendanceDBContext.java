    ...
    public List<MonthlyWageRecord> getMonthlyAttendanceByDepartment(int departmentId, int month, int year) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                ...
                MonthlyWageRecord monthlyWageRecord = recordsMap.getOrDefault(employeeId, new MonthlyWageRecord(employeeId, rs.getString("employeeName"), rs.getFloat("hourlyRate")));

                Date date = rs.getDate("attendanceDate");
                float alphaFactor = rs.getFloat("alphaFactor");
                monthlyWageRecord.addDailyEffort(date, alphaFactor);

                recordsMap.put(employeeId, monthlyWageRecord);
            }
